...
bool Mpu9250::Begin() {
  ...
  /* Check the WHO AM I byte first to identify the chip */
  if (!ReadRegisters(WHOAMI_, sizeof(who_am_i_), &who_am_i_)) {
    Serial.println(" -> Check WHOAMI: Read failed!");
    return false;
  }
  if ((who_am_i_ != WHOAMI_MPU9250_) && (who_am_i_ != WHOAMI_MPU9255_) && (who_am_i_ != WHOAMI_MPU6500_)) {
    Serial.printf(" -> Check WHOAMI: Invalid WHOAMI byte! Value: 0x%02X (Expected: 0x70, 0x71 or 0x73)\n", who_am_i_);
    return false;
  }
  ...
  if (has_magnetometer_) {
    ...
    if (who_am_i_ != WHOAMI_AK8963_) {
      Serial.printf(" -> Check AK8963 WHOAMI: Invalid AK8963 WHOAMI byte! Value: 0x%02X (Expected: 0x48)\n", who_am_i_);
      return false;
    }
    Serial.printf(" -> Check AK8963 WHOAMI: Valid AK8963 WHOAMI: 0x%02X\n", who_am_i_);
  ...
  if (!ConfigAccelRange(AccelRange::ACCEL_RANGE_16G)) {
    return false;
  }
  /* Set the gyro range to 2000DPS by default*/
  if (!ConfigGyroRange(GyroRange::GYRO_RANGE_2000DPS)) {
    return false;
  }
  /* Set the DLPF to 184HZ by default */
  if (!ConfigDlpfBandwidth(DlpfBandwidth::DLPF_BANDWIDTH_184HZ)) {
    return false;
  }
...
bool Mpu9250::EnableWom(int16_t threshold_mg, const WomRate wom_rate) {
  ...
  if (!WriteRegister(ACCEL_CONFIG2_, static_cast<uint8_t>(DlpfBandwidth::DLPF_BANDWIDTH_184HZ))) {
    return false;
  }
  ...
  uint8_t wom_threshold = static_cast<uint8_t>(threshold_mg / 4);
...
bool Mpu9250::Read() {
  ...
  accel_[0] = (static_cast<float>(accel_cnts_[1]) * accel_scale_) * G_TO_MPS2;
  accel_[1] = (static_cast<float>(accel_cnts_[0]) * accel_scale_) * G_TO_MPS2;
  accel_[2] = (static_cast<float>(accel_cnts_[2]) * accel_scale_ * -1.0f) * G_TO_MPS2;
  temp_ = (static_cast<float>(temp_cnts_) - 21.0f) / TEMP_SCALE_ + 21.0f;
  gyro_[0] = (static_cast<float>(gyro_cnts_[1]) * gyro_scale_) * DEG_TO_RAD;
  gyro_[1] = (static_cast<float>(gyro_cnts_[0]) * gyro_scale_) * DEG_TO_RAD;
  gyro_[2] = (static_cast<float>(gyro_cnts_[2]) * gyro_scale_ * -1.0f) * DEG_TO_RAD;
...
int8_t Mpu9250::ReadFifo() {
  ...
  for (int8_t i = 0; i < frames_to_read; i++) {
    ...
    fifo_gx_[i] = (static_cast<float>(gyro_cnts_[1]) * gyro_scale_) * DEG_TO_RAD;
    fifo_gy_[i] = (static_cast<float>(gyro_cnts_[0]) * gyro_scale_) * DEG_TO_RAD;
    fifo_gz_[i] = (static_cast<float>(gyro_cnts_[2]) * gyro_scale_ * -1.0f) * DEG_TO_RAD;
  }
...
bool Mpu9250::WriteRegister(uint8_t reg, uint8_t data) {
  ...
  if (iface_ == Interface::I2C) {
    ...
    if (err != 0) {
      Serial.printf(" -> WriteRegister(0x%02X, 0x%02X): I2C endTransmission returned error code %d\n", reg, data, err);
      return false;
    }
  } else {
  ...
  if (!ReadRegisters(reg, sizeof(ret_val), &ret_val)) {
    Serial.printf(" -> WriteRegister(0x%02X, 0x%02X): ReadRegisters failed during validation!\n", reg, data);
    return false;
  }
  if (data == ret_val) {
    return true;
  } else {
    Serial.printf(" -> WriteRegister(0x%02X, 0x%02X) failed validation! Read back: 0x%02X\n", reg, data, ret_val);
    return false;
  }
}
