...
class Mpu9250 {
 public:
  ...
  enum class DlpfBandwidth : int8_t {
  ...
  */
  ...
  uint8_t asa_buff_[3] = {0};
  float mag_scale_[3] = {0};
  uint8_t who_am_i_ = 0;
  ...
  bool has_magnetometer_ = true;
  /* Data */
  bool new_imu_data_ = false;
  bool new_mag_data_ = false;
  bool mag_sensor_overflow_ = false;
  uint8_t mag_data_[8] = {0};
  uint8_t data_buf_[23] = {0};
  int16_t accel_cnts_[3] = {0};
  int16_t gyro_cnts_[3] = {0};
  int16_t temp_cnts_ = 0;
  int16_t mag_cnts_[3] = {0};
  Vector3f accel_, gyro_, mag_;
  float temp_ = 0.0f;
  /* FIFO data */
  #if !defined(DISABLE_MPU9250_FIFO)
  bool fifo_overflow_ = false;
  int8_t fifo_num_frames_ = 0;
  int16_t fifo_bytes_ = 0;
